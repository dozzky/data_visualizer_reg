...
if uploaded_file:
    ...
    if not filtered_df.empty:
        ...
        if st.button("Построить графики (среднее значение)"):
            ...
            avg_kisvr_per_day_shift = kisvr_df.groupby(["Дата", "НомерСмены"])["Коэф. использования по времени (Кисвр)"].mean().reset_index()
            avg_kisvr_per_day_shift["Смена"] = avg_kisvr_per_day_shift["НомерСмены"].apply(lambda x: f"{x} смена")
